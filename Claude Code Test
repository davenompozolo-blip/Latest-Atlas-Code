...
def calculate_optimal_weights_var_cvar(
    ...
    def objective(weights):
        ...
        # Sharpe Ratio (assume risk-free rate ≈ 0 for simplicity, or use 0.04 for 4%)
        risk_free_rate = 0.04
    ...
    try:
        ...
        if not result.success:
            ...
            if result.status == 9 or abs(result.fun) > 0:  # Status 9 = iteration limit
                print(f"   ✓ Using best result found (not fully converged)")
...
def create_sector_allocation_treemap(df):
    ...
    fig.update_layout(
        title="🗺️ Portfolio Treemap - Hierarchical View",
        height=600,
...
def create_sector_allocation_sunburst(df):
    ...
    fig.update_layout(
        title="☀️ Portfolio Sunburst - Interactive Drill-Down",
        height=600,
        margin=dict(l=10, r=10, t=60, b=10)
    )
...
def create_optimization_comparison_chart(optimization_results):
    ...
    fig.update_layout(
        title="📊 Current vs Optimized Portfolio Metrics",
        yaxis_title="Value",
        xaxis_title="",
        height=400,
        showlegend=True,
...
def main():
    ...
    # ========================================================================
        ...
        col6.metric("📊 Positions", len(enhanced_df))
        ...
        if optimization_results and optimization_results.get('optimization_success'):
            st.success(f"""
            ✅ **Portfolio Optimization Active** - CVaR minimized by {abs(optimization_results['cvar_improvement_pct']):.1f}%
    ...
    # ========================================================================
        ...
        with st.expander("🎯 Tactical Allocation Guidance"):
        ...
        with tab6:
            st.markdown("#### 💵 US Treasury Yields & Forward Curves")
            st.info("📊 **Key Insight:** Monitor the yield curve for recession signals and forward rates for market expectations")
            ...
            st.markdown("##### 📊 Current Treasury Yields")
            ...
            st.markdown("---")
            st.markdown("##### 💡 Understanding Forward Rates")
            st.markdown("""
    ...
    # ========================================================================
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs([
            "📊 Core Risk", "🎲 Monte Carlo", "🔬 Advanced Analytics", "⚡ Stress Tests", "🎯 Portfolio Optimization"
        ])
        ...
        with tab5:
            st.markdown("#### 🎯 Portfolio Optimization - Mean-Variance (Markowitz)")
            st.info("""
            💡 **Mean-Variance Optimization**: Find optimal portfolio weights to **maximize Sharpe Ratio**,
            ...
            **Why Sharpe Maximization?** Unlike CVaR minimization which is too conservative and sacrifices upside,
            ...
            with st.expander("⚙️ Optimization Settings - Institutional Grade", expanded=True):
                ...
                st.markdown("---")
                st.caption("💡 **Institutional Settings**: 3-6% position limits, 30% max turnover, 30-35 position target")
            ...
            if run_optimization or 'optimization_results' in st.session_state:
                ...
                if run_optimization:
                    with st.spinner("🔄 Running Mean-Variance optimization (Sharpe maximization)... This may take a moment."):
                        ...
                        if len(returns_data) < 2:
                            st.error("❌ Insufficient data for optimization. Need at least 2 assets with historical data.")
                        else:
                            returns_df = pd.DataFrame(returns_data)
                ...
                if 'optimization_results' in st.session_state:
                    ...
                    if opt_results.get('optimization_success'):
                        ...
                        # Rebalancing Trades
                        st.markdown("---")
                        st.markdown("### 📋 Recommended Rebalancing Trades")
                        ...
                        if 'rebalancing_trades' in st.session_state:
                            ...
                            if not trades_df.empty:
                                ...
                                summary_col1.metric("💰 Total Buys", format_currency(total_buy_value))
                                summary_col2.metric("💸 Total Sells", format_currency(total_sell_value))
                                summary_col3.metric("💵 Transaction Costs", format_currency(total_transaction_cost))
                                summary_col4.metric("📊 Net Investment", format_currency(net_investment))
                                ...
                                st.download_button(
                                    label="📥 Download Rebalancing Plan (CSV)",
                                    data=csv,
    ...
    # ========================================================================
        ...
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
            "🎯 Attribution", "🔄 Sector Rotation", "📊 Concentration", "📊 Sector Allocation",
            "🏆 Quality Scorecard", "🎯 Drift Monitor", "📊 Correlation"
        ])
        ...
        with tab4:
            st.markdown("### 📊 Sector Allocation - Multiple Views")
            st.info("💡 **Moved from Portfolio Home** - Enhanced sector visualization with full width for better visibility")

            # Multiple visualization options
            viz_tabs = st.tabs(["📊 Donut", "🗺️ Treemap", "☀️ Sunburst", "📈 Bar Chart"])

            with viz_tabs[0]:
                st.markdown("#### 📊 Sector Allocation Donut Chart")
                sector_donut = create_sector_allocation_donut(enhanced_df)
                if sector_donut:
                    st.plotly_chart(sector_donut, use_container_width=True)

            with viz_tabs[1]:
                st.markdown("#### 🗺️ Sector Treemap - Hierarchical View")
                sector_treemap = create_sector_allocation_treemap(enhanced_df)
                if sector_treemap:
                    st.plotly_chart(sector_treemap, use_container_width=True)

            with viz_tabs[2]:
                st.markdown("#### ☀️ Sector Sunburst - Interactive Drill-Down")
                sector_sunburst = create_sector_allocation_sunburst(enhanced_df)
                if sector_sunburst:
                    st.plotly_chart(sector_sunburst, use_container_width=True)

            with viz_tabs[3]:
                st.markdown("#### 📈 Sector Bar Chart - Ranked by Value")
        ...
        # ====================================================================
        with tab5:
            st.markdown("### 🏆 Portfolio Quality Analysis")
            ...
            # Quality insights
            with st.expander("📋 Quality Insights & Recommendations"):
                ...
                if not low_quality.empty:
                    ...
                    st.success("✅ All holdings are B-tier quality or above!")
        ...
        with tab6:
            ...
            if rebalance_needed:
                ...
                st.success("✅ Portfolio allocation is within target ranges")
            ...
            drift_df = pd.DataFrame([
                {
                    ...
                    'Status': '⚠️ REBALANCE' if data['status'] == 'REBALANCE' else '✅ OK'
                }
            ...
            if rebalance_needed:
                ...
                **Rebalancing Summary:**
                ...
                with st.expander("📋 Detailed Trade Recommendations by Position"):
            ...
            with st.expander("⚙️ Benchmark Settings"):
                st.markdown(f"""
                ...
                **Last Benchmark Update:**
                ...
                if st.button("🔄 Force Refresh Benchmark Weights"):
                    st.cache_data.clear()
        ...
        # ====================================================================
        with tab7:
            st.markdown("### 📊 Portfolio Correlation & Risk Clustering")
        ...
        if 'valuation_company' in st.session_state:
            ...
            if 'valuation_results' in st.session_state:
                ...
                if method in ['FCFF', 'FCFE'] and all(k in results for k in ['total_pv_cash_flows', 'pv_terminal']):
                    ...
                    st.info(f"💡 Waterfall visualization available for DCF methods only. Current method: {method}")
