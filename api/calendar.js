...
async function readCache(cacheKey) {
    ...
    try {
        ...
        if (!r.ok) {
            console.error('[calendar] cache READ failed http=' + r.status);
            return null;
        }
        ...
        console.error('[calendar] cache READ threw: ' + ((e && e.message) || e));
        return null;
    }
}
...
async function writeCache(cacheKey, payload, ttlMs) {
    ...
    try {
        ...
        if (!r.ok) {
            console.error('[calendar] cache WRITE failed http=' + r.status);
            return false;
        }
        return true;
    } catch (e) {
        console.error('[calendar] cache WRITE threw: ' + ((e && e.message) || e));
        return false;
    }
}
