...
async function readCache(cacheKey) {
    ...
    try {
        ...
        if (!r.ok) {
            console.error('[movers] cache READ failed http=' + r.status);
            return null;
        }
        ...
        console.error('[movers] cache READ threw: ' + ((e && e.message) || e));
        return null;
    }
}
...
async function writeCache(cacheKey, payload, ttlMs) {
    ...
    try {
        ...
        if (!r.ok) {
            console.error('[movers] cache WRITE failed http=' + r.status);
            return false;
        }
        return true;
    } catch (e) {
        console.error('[movers] cache WRITE threw: ' + ((e && e.message) || e));
        return false;
    }
}
