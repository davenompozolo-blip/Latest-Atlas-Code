-- ─────────────────────────────────────────────────────────────────────────────
-- VIEW: vw_screener patch — company names from equity_cache
--
-- Problem: assets.name = assets.symbol for all portfolio stocks (Alpaca
-- only provides ticker, not company name). vw_screener was returning
-- "AAPL" as the name field, causing "AAPL AAPL" in the UI.
--
-- Fix: extract Name from equity_cache, handling two payload formats:
--   1. Flat   (screener-market.js mkt_overview): payload->>'Name'
--   2. Nested (equity.js overview):              payload->'overview'->>'Name'
-- Also handles both flat and nested formats for all fundamental fields.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Fundamentals from equity_cache — handles both payload formats:
--   flat    (direct AV response):   payload->>'PERatio'
--   nested  (equity.js response):   payload->'overview'->>'PERatio'
fundamentals AS (
  SELECT DISTINCT ON (ec.symbol)
    ec.symbol,
    COALESCE(
      NULLIF(ec.payload->>'Name', ''),
      NULLIF(ec.payload->'overview'->>'Name', '')
    ) AS company_name,
    COALESCE(
      (ec.payload->>'PERatio')::numeric,
      (ec.payload->'overview'->>'PERatio')::numeric
    ) AS pe_ratio,
    COALESCE(
      (ec.payload->>'EVToEBITDA')::numeric,
      (ec.payload->'overview'->>'EVToEBITDA')::numeric
    ) AS ev_ebitda,
    COALESCE(
      (ec.payload->>'PriceToBookRatio')::numeric,
      (ec.payload->'overview'->>'PriceToBookRatio')::numeric
    ) AS pb_ratio,
    COALESCE(
      (ec.payload->>'DividendYield')::numeric,
      (ec.payload->'overview'->>'DividendYield')::numeric
    ) AS div_yield,
    COALESCE(
      (ec.payload->>'ReturnOnEquityTTM')::numeric,
      (ec.payload->'overview'->>'ReturnOnEquityTTM')::numeric
    ) AS roe,
    COALESCE(
      (ec.payload->>'RevenueGrowthYOY')::numeric,
      (ec.payload->'overview'->>'RevenueGrowthYOY')::numeric
    ) AS revenue_growth,
    COALESCE(
      (ec.payload->>'MarketCapitalization')::bigint,
      (ec.payload->'overview'->>'MarketCapitalization')::bigint
    ) AS market_cap_raw,
    COALESCE(
      ec.payload->>'Sector',
    ...
    COALESCE(
      ec.payload->>'Industry',
      ec.payload->'overview'->>'Industry'
    ) AS industry,
    COALESCE(
      ec.payload->>'Country',
      ec.payload->'overview'->>'Country'
    ) AS country,
    COALESCE(
      ec.payload->>'Exchange',
      ec.payload->'overview'->>'Exchange'
    ) AS exchange,
    COALESCE(
      (ec.payload->>'NextEarningsDate')::date,
      (ec.payload->'overview'->>'NextEarningsDate')::date
    ) AS next_earnings,
    COALESCE(
      (ec.payload->>'AnalystTargetPrice')::numeric,
      (ec.payload->'overview'->>'AnalystTargetPrice')::numeric
    ) AS analyst_target
  FROM equity_cache ec
  WHERE ec.endpoint IN ('overview', 'mkt_overview')
    AND ec.expires_at > now() - INTERVAL '48 hours'
  ORDER BY ec.symbol, ec.expires_at DESC
)
SELECT
  a.symbol,
  -- Use equity_cache Name if available; fall back to asset name only if it
  -- differs from the symbol (Alpaca sets name = symbol for most assets).
  COALESCE(
    NULLIF(f.company_name, ''),
    NULLIF(a.name, a.symbol),
    a.symbol
  )                                            AS name,
