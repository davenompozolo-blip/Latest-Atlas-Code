-- vw_nexus_holdings: wire real fundamentals + macro + Max DD into the unified
-- intelligence surface.
--   • Max DD          ← vw_screener.current_drawdown_pct (real)
--   • Rate Sens / FX  ← deterministic sector mapping
--   • Macro signal    ← rate sensitivity × price regime
--   • Regime fit      ← macro signal
--   • P/E, PEG, Beta, analyst target (→ DCF upside) ← equity_cache overview payload
--     (populated by scripts/sync-fundamentals.mjs via the /api/equity overview feed)
--   • Conviction re-normalises over whichever components have data (valuation
--     only counts once analyst upside is present).
...
base as (
  select
    ...
    case
      when f.analyst_target is not null and p.current_price > 0
      then round((f.analyst_target - p.current_price) / p.current_price * 100::numeric, 1)
    end as dcf_upside_pct,
    f.analyst_target as intrinsic_value,
