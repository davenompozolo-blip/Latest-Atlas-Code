-- Rebuild mv_cortex_screener:
--  * Universe = assets that have EITHER ≥20 days of price history OR a
--    fundamentals row in equity_cache. This lets the large/mid-cap universe
--    appear as soon as sync_fundamentals populates it, even before price
--    history is backfilled (ret_1m is NULL until prices arrive).
--  * Reads the ACTUAL equity_cache payload shape written by sync_fundamentals
--    v2: payload->'overview', payload->'metric', payload->>'market_cap_usd'.
--    Metric field names are best-effort Finnhub keys; the full metric blob is
--    stored, so these can be re-pointed without a re-fetch.
...
SELECT
  ...
  (f.payload->'metric'->>'evToEbitdaTTM')::numeric                         AS ev_ebitda,
  ...
  COALESCE(
    (f.payload->'metric'->>'roicTTM')::numeric,
    (f.payload->'metric'->>'roiTTM')::numeric
  )                                                                        AS roic,
  (f.payload->'metric'->>'roeTTM')::numeric                               AS roe,
  (f.payload->'metric'->>'totalDebt/totalEquityQuarterly')::numeric       AS debt_equity,
  (f.payload->'metric'->>'netDebtToTotalCapitalAnnual')::numeric          AS net_debt_ebitda,
