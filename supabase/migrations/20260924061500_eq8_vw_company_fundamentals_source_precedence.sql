create or replace view public.vw_company_fundamentals as
 WITH sect AS (
         ...
         SELECT i.symbol,
          ...
          WHERE i.source = (
                  -- ONE SOURCE PER PERIOD. The PK carries `source`, so a symbol
                  -- loaded from two providers yields two rows per fiscal year and
                  -- a surface reading "the latest row" gets whichever the planner
                  -- returns. Measured 2026-09-24: 129 duplicated (symbol,
                  -- fiscal_year) rows across 8 symbols the moment EDGAR landed
                  -- beside Alpha Vantage.
                  --
                  -- EDGAR wins because it IS the filing rather than a vendor's
                  -- normalisation of it, it resolves restatements explicitly
                  -- (latest filed value per period), and it covers the universe
                  -- where Alpha Vantage covers ten symbols at 25 requests/day.
                  --
                  -- The two agree where it matters: GOOGL, TGT and SNDK are
                  -- bit-identical across every overlapping year and ADBE differs
                  -- only by rounding. They DISAGREE materially on JPM (38.7% on
                  -- revenue) and PFE (88.9% on net income) -- a bank's net-versus-
                  -- gross revenue convention and restatement handling. Picking one
                  -- source per period is what stops those two answers reaching one
                  -- page; which of them is right is its own question.
                  SELECT p.source
                    FROM company_income_statement p
                   WHERE p.symbol = i.symbol
                     AND p.fiscal_date_ending = i.fiscal_date_ending
